...
# 1. Add imports
if "import { GenericModal, ModalConfig }" not in content:
    content = content.replace(
        "import { ProjectAdminDashboard } from '../components/ProjectAdminDashboard';",
        "import { ProjectAdminDashboard } from '../components/ProjectAdminDashboard';\nimport { GenericModal, ModalConfig } from '../components/GenericModal';\nimport { useProjectModules } from '../hooks/useProjectModules';"
    )

# 2. Uncomment useProjectModules
content = content.replace(
...
# 3. Add modalConfig state
if "const [modalConfig, setModalConfig] = useState" not in content:
    content = content.replace(
        "const [activeTab, setActiveTab] = useState('dashboard');",
        "const [activeTab, setActiveTab] = useState('dashboard');\n  const [modalConfig, setModalConfig] = useState<ModalConfig | null>(null);\n  const [corrFilter, setCorrFilter] = useState('');"
    )

# 4. Modify Sidebar (Re-order Correspondence, Remove Procurement, Subcontractors, rename Punch List)
new_sidebar = """{[
...
content = re.sub(r"\{\[\s*\{ id: 'dashboard',.*?\]", new_sidebar, content, flags=re.DOTALL)

# Remove Tabs Procurement and Subcontractors logic
content = re.sub(r"\{\/\* PUNCH LIST TAB \*\/\}.*?\{activeTab === 'punch_list'.*?\{\/\* CORRESPONDENCE TAB \*\/\}", "{/* CORRESPONDENCE TAB */}", content, flags=re.DOTALL)

# We will just write a function to replace the variations, snag_list, and correspondence blocks entirely.
# First, remove MOCK blocks from variations, correspondence, snag
# Let's replace the VARIATIONS block:
new_variations_block = """{/* VARIATIONS TAB */}
...
content = re.sub(r"\{\/\* VARIATIONS TAB \*\/\}.*?\{\/\* CORRESPONDENCE TAB \*\/\}", new_variations_block + "\n\n          {/* CORRESPONDENCE TAB */}", content, flags=re.DOTALL)
content = re.sub(r"\{\/\* VARIATIONS TAB \*\/\}.*?\{\/\* SUBCONTRACTORS TAB \*\/\}", new_variations_block + "\n\n          {/* SUBCONTRACTORS TAB */}", content, flags=re.DOTALL)


new_correspondence_block = """{/* CORRESPONDENCE TAB */}
...
content = re.sub(r"\{\/\* CORRESPONDENCE TAB \*\/\}.*?\{\/\* EQUIPMENT LOGS TAB \*\/\}", new_correspondence_block + "\n\n          {/* EQUIPMENT LOGS TAB */}", content, flags=re.DOTALL)

new_snags_block = """{/* SNAG LIST TAB */}
          {activeTab === 'snag_list' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
...
content = re.sub(r"\{\/\* SUBCONTRACTORS TAB \*\/\}.*?\{\/\* CORRESPONDENCE TAB \*\/\}", new_snags_block + "\n\n          {/* CORRESPONDENCE TAB */}", content, flags=re.DOTALL)
# if snag_list is still there at the old place
content = re.sub(r"\{\/\* PUNCH LIST TAB \*\/\}.*?\{\/\* CORRESPONDENCE TAB \*\/\}", new_snags_block + "\n\n          {/* CORRESPONDENCE TAB */}", content, flags=re.DOTALL)


# 5. GenericModal at the bottom
if "<GenericModal" not in content:
    content = content.replace(
        "        </div>\n      </div>\n    </div>\n  );\n};",

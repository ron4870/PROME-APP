...
xmlFileInput.addEventListener('change', () => {
  if (xmlFileInput.files.length > 0) {
    handleFile(xmlFileInput.files[0]);
  }
});
...
function handleFile(file) {
  ...
  reader.onload = async (e) => {
    ...
    selectedFileName.textContent = state.fileName;
    selectedFileSize.textContent = state.fileSize;
...
function drawMapTiles(ctx, cx, cy, canvasToMap, mapToCanvas, align) {
  ...
  // Calculate Web Mercator zoom based on pixel scale
  let z = Math.round(Math.log2(earthCircumference * state.zoom / 256));
...
function drawAlignment() {
  ...
  const align = state.transformedAlignment || state.alignments[state.activeAlignmentIndex];
  if (!align) return;
